[JSTools 초간단 사용법]
  - JSTools는 JavaScript 파일 배포를 위한 지원 도구 입니다.
  - 압축과 난독화 모듈은 YUICompressor를 사용했습니다.
  - 버그가 있으면 신고해주세요~
  - 도스 커맨드 상에서 실행할때는 아래와 같이 java를 이용해 실행한다.
  - 위키 설명: http://devcode.nhncorp.com/projects/jsassistant/wiki/JSTools%202.0%20%EC%86%8C%EA%B0%9C

[기본 문법]
 java -jar JSTools.jar <파일목록> [-옵션]
    - <파일목록> : 필수 (단, -conf 옵션을 사용해 설정할 경우 명시적으로 쓴 파일이 -conf 설정보다 우선한다.) 
    ...
    - [옵션] : 선택   
      ...
      -compress=[0,1,2,3,4]
        ...
        5 일 경우, UglifyJS로 압축한다. 압축율이 가장좋다.
           ex) java -jar JSTools.jar file1 file2 dir1 output=./result.js  ---> 옵션을 지정하지않아도 compress의 기본값은 0이다.
      ...
      -jslint[=옵션파일지정] : JSlint를 실행한다.
        * 옵션파일을 지정해, jslint를 설정한다. 자세한 내용은 첨부된 jslint.conf 파일 참고  
        * 옵션을 지정하면 JSLint를 수행한다.
...
3. 템플릿 ID 지정 (단, ID 사이에 공백 있으면 안됨)
	<!--[템플릿_ID]-->
		템플릿 코드 
	<!--[/템플릿_ID]-->
...
==================
FAQ,. 
1. Ant 실행시 이클립스 Console 창에서 한글이 깨질 경우 
...
2. 그냥 이클립스 Console 창에서 한글이 깨지는 경우 
