...
public class TestRemoveComment extends TestCase {
	...
	public void testSingleLineComment() throws IOException {
		assertEquals("var b = e.protocol + \"//\" + e.host; ",
			RemoveComment.removeSingleLineComment("var b = e.protocol + \"//\" + e.host; //.이거 주석입니다."));
	...
	public void testHasLink() {
		...
		assertEquals("url: \" http://static.naver.com/maps2/icons/path_m2.png\", ",
			RemoveComment.removeSingleLineComment("url: \" http://static.naver.com/maps2/icons/path_m2.png\", //아하하 "));
		assertEquals("url: \" http://static.naver.com/maps2/icons/path_m2.png\", ",
			RemoveComment.removeSingleLineComment("url: \" http://static.naver.com/maps2/icons/path_m2.png\", "));
	}

	public void testHasRegExp() {
		assertEquals("if (/^\\.\\//.test(a)) {	", RemoveComment.removeSingleLineComment("if (/^\\.\\//.test(a)) {	"));
		assertEquals("if (/^\\.\\//.test(a)) {	", RemoveComment.removeSingleLineComment("if (/^\\.\\//.test(a)) {	// 이것도 지워!!"));
		assertEquals("var sProtocol = (this._getLocation()).match(/^[a-zA-Z]+:\\/\\//);",
			RemoveComment.removeSingleLineComment("var sProtocol = (this._getLocation()).match(/^[a-zA-Z]+:\\/\\//);"));
	}

	public void testHasFlashObject() {
		assertEquals(
			"pluginspage=\"http://www.macromedia.com/go/getflashplayer\" width=\"1\" height=\"1\" allowScriptAccess=\"always\" swLiveConnect=\"true\" FlashVars=\"activeCallback='+activeCallback+'\"></embed></object></div>');",
	...
	public void testBlockComment3() {
		assertEquals(
			"}, width = WD - margin.left - margin.right, height = HT - margin.top - margin.bottom;",
			RemoveComment.removeBlockComment("}, width = /*942/2*/WD - margin.left - margin.right, height = /*400/2*/HT - margin.top - margin.bottom;"));
	}
	...
	public void testException2() {
		assertEquals("oOpt.price = String(oOpt.price).replace(/(\\d)(?=(?:\\d{3})+(?!\\d))/g,'$1,');	",
			RemoveComment.removeSingleLineComment("oOpt.price = String(oOpt.price).replace(/(\\d)(?=(?:\\d{3})+(?!\\d))/g,'$1,');	// 3자리씩 콤마 찍기"));
	}
